...
public class ClientHandler extends Thread {
    ...
    private void handleLogin(String request, BufferedWriter writer) throws IOException {
        System.out.println("[SERVER] handleLogin 요청: " + request);
        ...
        if (loggedInUsers.contains(id)) {
            ...
            // 기존 핸들러가 완전히 종료될 때까지 대기 루프 제거 (즉시 진행)
        }
    ...
    private void handleCreateChatRoom(BufferedReader reader, BufferedWriter writer) throws IOException {
        System.out.println("[SERVER] handleCreateChatRoom 요청 수신");
        ...
        writer.flush();
        System.out.println("[SERVER] 채팅방 생성 결과: " + (created ? "성공" : "실패"));
    }
    ...
    private void handleSendMessage(String request, BufferedWriter writer) throws IOException {
        System.out.println("[SERVER] handleSendMessage 요청: " + request);
        ...
        if (room == null || msg == null || userId == null) {
            ...
            writer.flush();
            System.out.println("[SERVER] 메시지 파일 저장 결과: 실패 (필수 파라미터 누락)");
            return;
        }
        ...
        if (!file.exists()) {
            writer.write("SEND_MESSAGE_FAIL\n");
            writer.flush();
            System.out.println("[SERVER] 메시지 파일 저장 결과: 실패 (파일 없음)");
            return;
        }
        try (BufferedWriter fileWriter = new BufferedWriter(new FileWriter(file, true))) {
            fileWriter.write(userId + ": " + msg + System.lineSeparator());
            writer.write("SEND_MESSAGE_SUCCESS\n");
            System.out.println("[SERVER] 메시지 파일 저장 결과: 성공");
        } catch (IOException e) {
            writer.write("SEND_MESSAGE_FAIL\n");
            System.out.println("[SERVER] 메시지 파일 저장 결과: 실패 (IOException)");
        }
        writer.flush();
    }
    ...
    private void handleGetChatHistory(String request, BufferedWriter writer) throws IOException {
        System.out.println("[SERVER] handleGetChatHistory 요청: " + request);
    ...
    private void handleGetMyChatRooms(String request, BufferedWriter writer) throws IOException {
        System.out.println("[SERVER] handleGetMyChatRooms 요청: " + request);
        String userId = null;
