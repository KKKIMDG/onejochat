...
public class ChatController {
        ...
        createChatView.getNormalButton().addActionListener(e -> {
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() {
                    ...
                    if (created) {
                        // 생성 성공 시 채팅방 목록 갱신 및 목록 뷰로 이동
                        SwingUtilities.invokeLater(() -> {
                            JFrame topFrame = (JFrame) SwingUtilities.getWindowAncestor(mainPanel);
                            if (topFrame instanceof view.MainFrame) {
                                ((view.MainFrame) topFrame).refreshChatListView();
                                ((view.MainFrame) topFrame).getCardLayout().show(((view.MainFrame) topFrame).getMainPanel(), "chatRoomListView");
                            }
    ...
    private boolean sendCreateChatRoomRequest(String roomName, List<String> invitedFriends) {
        if (writer == null) {
            System.out.println("[CLIENT] 서버 연결 없음");
            SwingUtilities.invokeLater(() ->
                    JOptionPane.showMessageDialog(null, "서버 연결이 없습니다."));
            return false;
        }
        try {
            System.out.println("[CLIENT] 채팅방 생성 요청: roomName=" + roomName + ", invited=" + invitedFriends);
            ...
            System.out.println("[CLIENT] 채팅방 생성 응답: " + response);
            if (response == null) {
                SwingUtilities.invokeLater(() ->
                        JOptionPane.showMessageDialog(null, "서버로부터 응답이 없습니다. 서버가 실행 중인지, 네트워크가 정상인지 확인하세요."));
                return false;
            }
            if ("CREATE_CHATROOM_SUCCESS".equals(response)) {
                ...
                SwingUtilities.invokeLater(() ->
                        JOptionPane.showMessageDialog(null, "채팅방 생성 요청은 성공했지만 파일 저장 실패: " + response));
                return false;
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            System.out.println("[CLIENT] 채팅방 생성 예외: " + ex.getMessage());
            SwingUtilities.invokeLater(() ->
                    JOptionPane.showMessageDialog(null, "서버에 채팅방 생성 요청 실패: " + ex.getMessage()));
            return false;
        }
    }
