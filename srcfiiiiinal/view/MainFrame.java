...
public class MainFrame extends JFrame {
    ...
    public MainFrame(Socket socket) {
        ...
        homeView.getAddFriendButton().addActionListener(e -> {
            ...
            new FriendAddController(friendAddView, friendService, userService, myId, this);
    ...
    private void loadFriendsFromFile(String userId) {
        myFriends.clear();
        homeView.clearFriendList(); // 홈 뷰의 친구 목록도 초기화 (clearFriendList 메서드 필요)
        try (
            java.net.Socket tempSocket = new java.net.Socket("localhost", 9001);
            java.io.PrintWriter writer = new java.io.PrintWriter(tempSocket.getOutputStream(), true);
            java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.InputStreamReader(tempSocket.getInputStream()))
        ) {
            writer.println("GET_FRIENDS:" + userId);
            writer.flush();
            String line;
            while ((line = reader.readLine()) != null) {
                if ("END_OF_FRIENDS".equals(line)) break;
                if (line.startsWith("FRIEND:")) {
                    String friendId = line.substring("FRIEND:".length());
                    myFriends.add(friendId);
                    homeView.addFriendToList(friendId);
                }
            }
            // 별명 파일도 항상 최신화
            homeView.reloadNicknamesAndRepaint();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
