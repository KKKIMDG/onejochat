...
public class ClientHandler extends Thread {
    ...
    private boolean createChatRoomFile(String ownerId, String roomName, String invited) {
        ...
        try (BufferedWriter fileWriter = new BufferedWriter(new FileWriter(filename))) {
            ...
            // invited에 ownerId가 포함되어 있는지 확인 후, 없으면 추가
            List<String> participants = new ArrayList<>();
            participants.add(ownerId);
            for (String id : invited.split(",")) {
                String trimmed = id.trim();
                if (!trimmed.isEmpty() && !trimmed.equals(ownerId)) {
                    participants.add(trimmed);
                }
            }
            fileWriter.write("참여자: " + String.join(",", participants) + System.lineSeparator());
    ...
    private boolean addFriendToFile(String fromId, String toId) {
        try {
            // 1. friends.txt에 기록 (로그 용도)
            try (BufferedWriter fw = new BufferedWriter(new FileWriter("friends.txt", true))) {
                fw.write("FRIEND:" + fromId + "," + toId + System.lineSeparator());
            }

            // 2. fromId의 friends_FROMID.txt에 toId 추가 (중복 방지)
            File fromFile = new File("friends_" + fromId + ".txt");
            boolean already = false;
            if (fromFile.exists()) {
                try (BufferedReader r = new BufferedReader(new FileReader(fromFile))) {
                    String line;
            ...
            if (!already) {
                try (BufferedWriter w = new BufferedWriter(new FileWriter(fromFile, true))) {
                    w.write(toId + System.lineSeparator());
                }
            }

            // 3. toId의 friends_TOID.txt에 fromId 추가 (상호 친구, 중복 방지)
            File toFile = new File("friends_" + toId + ".txt");
            already = false;
            if (toFile.exists()) {
                try (BufferedReader r = new BufferedReader(new FileReader(toFile))) {
                    String line;
                    while ((line = r.readLine()) != null) {
                        if (line.trim().equals(fromId)) {
                            already = true;
                            break;
                        }
                    }
            ...
            if (!already) {
                try (BufferedWriter w = new BufferedWriter(new FileWriter(toFile, true))) {
                    w.write(fromId + System.lineSeparator());
                }
            }
    ...
    /**
     * 친구 목록 요청을 처리합니다.
     * @param request 클라이언트로부터 받은 요청 (GET_FRIENDS:ID=...)
     * @param writer 클라이언트에게 응답을 보내기 위한 BufferedWriter
     * @throws IOException 입출력 예외
     */
    private void handleGetFriends(String request, BufferedWriter writer) throws IOException {
        // 요청에서 ID 추출
        String id = request.substring("GET_FRIENDS:".length()).trim();
        File file = new File("friends_" + id + ".txt");
        if (!file.exists()) {
            writer.write("END_OF_FRIENDS\n");
            writer.flush();
            return;
        }
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
